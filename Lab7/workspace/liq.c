...
int builtin_cmd(char **argv)
{
    if(argv[0] == NULL) return 1; // if input nothing (\n), return True
    if(!strcmp(argv[0], "quit")) exit(0); //quit--->exit
    if(!strcmp(argv[0], "fg") || !strcmp(argv[0], "bg"))
    {
        /*command for bg and fg*/
        do_bgfg(argv);
        return 1;
    }
...
void waitfg(pid_t pid)
{
    while(pid == fgpid(jobs))   // 前台进程等待至后台进程结束
    {
        sleep(0);
    }
    return;
...
void sigchld_handler(int sig)
{
    ...
    int status;
    pid_t pid;
    while((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) //子进程中有被停止的进程,则开始循环
    {
        if (WIFEXITED(status))      // 子进程是由exit命令停止的，则删除该jobs中的相应子进程
        {
            deletejob(jobs, pid);
        }
        else if (WIFSIGNALED(status))      //该子进程是因为一个未捕获的信号终止的，则删除该jobs中的相应子进程
        {
            deletejob(jobs, pid);
        }
        else if (WIFSTOPPED(status))      //返回的子进程当前是停止的， 将job的状态设为停止
        {
...
void sigint_handler(int sig)
{
    ...
    if (pid != 0)    //有一个前台进程
    {
        if (kill(-pid, sig) < 0)  //将pid设为负数，用kill函数将信号返回到进程组pid信号所在的进程组中的所有子进程中
        {
            unix_error("error when kill in sigint_handler");//kill函数返回负数--->进程停止失败
        }
...
void sigtstp_handler(int sig)
{
    pid_t pid = fgpid(jobs);//equal to sigint_handler
    int job_id = pid2jid(pid);
    if (pid != 0)
    {
        if (kill(-pid, sig) < 0)
